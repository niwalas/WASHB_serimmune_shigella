...
df2 <- const_foi(ages, lambda = model_ppc2$`2.5%`[2], 
                 gamma = model_ppc2$`2.5%`[1]) #gamma 2.5%
df3 <- const_foi(ages, lambda = model_ppc2$`97.5%`[2], 
                 gamma = model_ppc2$`97.5%`[1]) #gamma 2.5%

df4 <- left_join(df1, df2 %>% select(age, prop_x2) %>% rename(low_ci = prop_x2), by = "age")
df5 <- left_join(df4, df3 %>% select(age, prop_x2) %>% rename(up_ci = prop_x2), by = "age")
...
p_obs <- serop2 %>% rename(ages = age_mo) %>% mutate(prev = prev*100, 
                                   ...
                                   low = lower*100)
supp_fig <- ggplot() +
  geom_line(data = df5, aes(x = age, y = prop_x2*100), 
  ...
  geom_ribbon(data = df5, aes(x = age, y = prop_x2*100, 
                              ymin = low_ci*100, 
                              ymax = up_ci*100),
                  alpha = 0.2, 
                  color = "#0072B2",
...
fig_m <- (m1 / plot_spacer() / m2 + plot_layout(heights = c(1, 0.2, 1))) 
fig_m2 <- ggdraw(fig_m) +
